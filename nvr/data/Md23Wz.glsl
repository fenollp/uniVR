// Shader downloaded from https://www.shadertoy.com/view/Md23Wz
// written by shadertoy user Dave_Hoskins
//
// Name: Mars Jetpack
// Description: Uses sphere tracing to accumulate normals across the landscape. Materials are calculated after the tracing loop, so only the normal can be used as reference. Sphere diameter to create depth of field is distance squared.  http://youtu.be/2eSb8zB4dBo
...
//	x.y += 83.2;
	...
	float a = texture2D( iChannel0, (p+vec2(0.5,0.5))*STEP, -100.0 ).x;
	float b = texture2D( iChannel0, (p+vec2(1.5,0.5))*STEP, -100.0 ).x;
	float c = texture2D( iChannel0, (p+vec2(0.5,1.5))*STEP, -100.0 ).x;
	float d = texture2D( iChannel0, (p+vec2(1.5,1.5))*STEP, -100.0 ).x;
