// Shader downloaded from https://www.shadertoy.com/view/4tl3RM
...
float Map( vec3 p )
{
    ...
    // Beak...
    d = sMin(d,Segment(p-vec3(0.0, 1.,2.5),vec3(0.0, 0.0,-.4), vec3(.0, animParts.w*.2,.3), .12, .12), .2);
    d = sMin(d,Segment(p-vec3(0.0, 1.0,2.5),vec3(0.0, -animParts.w*.25,-.5), vec3(.0, -animParts.w,.15), .1, .1), .05);
    // Eyes...
    p.x = abs(p.x);
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    animParts.w = TweetVolume(gTime)*.8;
