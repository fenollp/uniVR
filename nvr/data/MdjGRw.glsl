// Shader downloaded from https://www.shadertoy.com/view/MdjGRw
// written by shadertoy user TekF
//
// Name: Mesas
// Description: Just sketching.<br/><br/>Try uncommenting #define INTERPOLATION_FIX if the ground has stripy artefacts.
// Ben Weston 2013
...
vec3 Shade( vec3 pos, vec3 norm, float dist )
{
	...
	// shadow
	vec2 interval = vec2(.01,(.2-pos.y)/sunDir.y);
	float shadow = 1.0;
	if ( ndotl > 0.0 )
	{
...
{
	float T = iGlobalTime*.2+iMouse.x*.1;
