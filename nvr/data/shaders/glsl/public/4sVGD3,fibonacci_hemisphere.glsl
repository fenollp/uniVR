// Shader downloaded from https://www.shadertoy.com/view/4sVGD3
// written by shadertoy user paniq
//
// Name: Fibonacci Hemisphere
// Description: randomly distributing fibonacci balls on a hemisphere, seeding phase from UV. Wrote this to better understand SSAO sampling.
#define M_PI 3.1415926535898

vec3 fibonacci_point (int numsamples, int sample, float seed) {
	float fib_incr = M_PI * (3.0 - sqrt(5.0));
    float numsamplesf = float(numsamples);
    float rnd = seed * numsamplesf;
    
    float offset = 1.0 / numsamplesf;
    float samplef = float(sample);
    
    float z = 1.0 - (offset * (samplef - 0.5));
    float r = sqrt(1.0 - z * z);
    float phi = mod(samplef + rnd, numsamplesf) * fib_incr;
...
float sqdFibonacciPoint (vec3 p, int numsamples, int sample) {
    vec3 c = fibonacci_point(numsamples, sample, seed);
    p -= c;
    return dot(p,p);
...
vec2 doModel( vec3 p ) {
    float d = sdFibonacciHemisphere(p.xzy);
...
vec4 doMaterial( in vec3 pos, in vec3 nor )
{
    ...
    return mix(vec4(1.0,1.0,1.0,0.1), //nor * 0.5 + 0.5,
