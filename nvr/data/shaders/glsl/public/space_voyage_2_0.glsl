// Shader downloaded from https://www.shadertoy.com/view/Xd3SRr
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    //shake
    uv += (texture2D(iChannel1,vec2(0.,iGlobalTime)).xy - vec2(.5)) *.01 * (sin(iGlobalTime)*.5+1.);
    
    vec4 col = texture2D(iChannel0,uv * vec2(.59,1.)) ;
    //oscillation
    uv = uv * 3. + vec2(-.5);
