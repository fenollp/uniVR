// Shader downloaded from https://www.shadertoy.com/view/XssGz8
...
vec3 render( vec2 uv )
{
    ...
    return srgb2lin(texture2D( iChannel0, uv ).rgb );
}
...
{	
    ...
    if ( uv.x > 0.6 &&  uv.y < 0.2 )
    {
        vec2 luv = remap( uv, vec2(0.6,0.0), vec2(1.0, 0.2) );
        vec3 c;
        ...
        c += step( abs(rgb-luv.yyy), vec3(0.01) );
