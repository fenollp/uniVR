// Shader downloaded from https://www.shadertoy.com/view/XdcSW2
// written by shadertoy user vox
//
// Name: Angrier Flowers
// Description: Angrier Flowers
...
vec2 spiral(vec2 uv, out float layer, float turns)
{
    ...
    uv = saw(vec2(r*PI-time*PI, theta+time));
    return uv;
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    uv = spiral(uv, z, 3.0+saw(time));
    uv = rotatez(vec3(uv, 0.0), time).xy;
        map *= 1.0+square(uv*2.0-1.0, 1.0, z*PI);
    uv = spiral(uv*2.0-1.0, z, 4.0+saw(time));
    ...
    for(int i = 0; i < max_iterations; i++)
    {
        ...
        map *= 1.0+iterations*square(uv, iterations, z);
        uv = saw(tree(uv)*PI); 
        
        noise += map;
        
    }
    map = log(noise+z)*PI + time/PI;
    ...
    map = log(noise*.9+z)*2.0*PI;
    float black = pow(1.0/2.0, -abs(tan(saw(time*PI+map)*PI-PI/2.0)));//atan(1.0/sin(time*.9+map/GR*E))/PI+.5;
    map = log(noise+z)*1.0*PI;
    float white = pow(1.0/2.0, abs(tan(saw(time*GR+map)*PI-PI/2.0)));//atan(1.0/sin(time*.9+map/GR*E))/PI+.5;
    
    fragColor *= black;
    fragColor += white*(1.0-black);
    fragColor.a = 1.0;
}