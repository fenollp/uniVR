// Shader downloaded from https://www.shadertoy.com/view/ldKSzR
// written by shadertoy user vox
//
// Name: Dancing Paint
// Description: Dancing Paint
//-----------------SETTINGS-----------------
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    seedling = height*8.0*PI-time;
    
    uv = rotatez(uv*2.0-1.0, seedling)*.5+.5;
    ...
    if(ifsPowed > 1.0-height/GR)
    {
        if( sound < .125)
            fragColor = vec4(sound);// = 0.0;
        else
        	discard;
        return;
    }
    ...
    //depth /= float(max_i);
    float shift = time*PI+seedling;

    float stripes = depth*1.0+simplex3d(vec3(fragCoord.xy/iResolution.xy+uv, seedling));
    float black = smoothstep(0.0, .25, saw(stripes-shift));
    float white = smoothstep(0.75, 1.0, saw(stripes-shift));
    ...
    fragColor = vec4(final, 1.0)*smoothstep(0.0, .1, height*sound*(1.0-ifsPowed));
