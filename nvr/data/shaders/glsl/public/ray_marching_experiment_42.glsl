// Shader downloaded from https://www.shadertoy.com/view/MdtXRl
// written by shadertoy user aiekick
//
// Name: Ray Marching Experiment 42
// Description: based on [url=https://www.shadertoy.com/view/ldtSRl]2D Rad Rep : Particles 2 (196c)[/url]
//    mouse for see the patter, frozen
...
vec3 effect(vec2 g) 
{
	...
	g.x *= floor(g.y);
    if (iMouse.z <= 0.)
		g.x += fract(g.y) * sin(iGlobalTime) * 4.;
	g.x += floor(g.y) * iGlobalTime * .2;
	g = fract(g) - .5;
	return vec3(.06,.1,0.04)/dot(g,g);
}
...
vec4 map(vec3 p)
{
    ...
    float m = length(p);
    float me = m - 4.77 + disp.x;
    float mi = m - 4.48 - disp.x;
    return vec4(max(-mi, me), disp.yzw);
}
...
void mainImage( out vec4 f, in vec2 g )
{
    ...
    vec3 ro = vec3(-sin(cam_a)*cam_d, 6.52*sin(cam_a), cos(cam_a)*cam_d); //
