// Shader downloaded from https://www.shadertoy.com/view/XdcSRH
// written by shadertoy user Mr_E
//
// Name: Strict Cube Field
// Description: This is a strict Cube Field, unlike my last one, due to the rigidness of the edges of the sides of the cubes. This is also another test for 3 dimensional shapes.
...
{
    vec3 cam_pos = vec3(cos(iGlobalTime/5.)*20.0,sin(iGlobalTime/5.)*40.0,20.0);
    ...
    if (f<maxd) {
        ...
        //fragColor = vec4(b, 0.1, sin(b), cos(iGlobalTime*iGlobalTime));//Set the screen pixel to that color
    } else {
