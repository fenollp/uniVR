// Shader downloaded from https://www.shadertoy.com/view/ld3SRr
...
vec4 MAT_triplanarTexturing(vec3 p, vec3 n)
{
    ...
    vec4 cX = abs(n.x)*texture2D(iChannel1,p.zy);
    vec4 cY = abs(n.y)*texture2D(iChannel1,p.xz);
    vec4 cZ = abs(n.z)*texture2D(iChannel1,p.xy);
...
{
    vec4 sliderVal = texture2D(iChannel0,vec2(0,0));
    ...
    {
    	return texture2D(iChannel0,fragCoord.xy/iResolution.xy);
    }
    return vec4(0);
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    float rotX = ((iMouse.z>0.)&&any(lessThan(iMouse.xy/iResolution.xy,vec2(0.9,0.80))))?
	             ((iMouse.x/iResolution.x)*2.0*3.14) : (iGlobalTime*0.3);
