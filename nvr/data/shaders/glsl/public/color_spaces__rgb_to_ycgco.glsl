// Shader downloaded from https://www.shadertoy.com/view/4lBXDR
// written by shadertoy user 4rknova
//
// Name: Color Spaces: RGB to YCgCo
// Description: Conversion between RGB and YCgCo color spaces.
//    Delta shows the conversion loss.
//    Click on the respective cell to view it in full size.
...
#ifdef TEXT
// Font rendering based on FlyGuy's shader: https://www.shadertoy.com/view/llSGRm
#define FONTSC_SZ vec2(2.5, 5)     // Font size
#define SCREEN_SZ vec2(1024, 768) // Screen size
...
float Text(vec2 uv)
{
    if (iMouse.w > .5) return 0.;
    
    uv *= SCREEN_SZ;
    float r = 0.;
    
    carret = vec2(10,720.5-2.*CHR.y);
	r += T(c_C) + T(c_O) + T(c_N) + T(c_V) + T(c_E) + T(c_R) + T(c_S) + T(c_I) + T(c_O) + T(c_N);
    carret = vec2(275,720.5-2.*CHR.y);
	r += T(c_D) + T(c_E) + T(c_L) + T(c_T)
       + T(c_A) + T(c_lbr) + T(c_x) 
       + T(c_5) + T(c_0)   + T(c_0)
       + T(c_0) + T(c_rbr);
    carret = vec2(530,720.5-2.*CHR.y);
    r += T(c_Y);
    carret = vec2(530,460.5-2.*CHR.y);
	r += T(c_C) + T(c_G);
    carret = vec2(530,200.5-2.*CHR.y);
	r += T(c_C) + T(c_O);
...
{
	float Y  =  .25 * rgb.x + .50 * rgb.y + .25 * rgb.z; // Luminance
	float Cg = -.25 * rgb.x + .50 * rgb.y - .25 * rgb.z; // Chrominance Green
	float Co =  .50 * rgb.x - .50 * rgb.z;               // Chrominance Orange
    return vec3(Y,Cg,Co);
}
...
{
	vec2 uv = p.xy / iResolution.xy, st = uv;

    // Transformations
    vec3 r = texture2D(iChannel0, st).xyz;
    vec3 f = RGBtoYCgCo(r);
    vec3 b = YCgCotoRGB(f);
    ...
    vec3 cg = vec3(f.y + .5);
    vec3 co = vec3(f.z + .5);
    vec3 d  = vec3(length(b-r) * 5000.);
    
    vec2 e = (iMouse.w > .5) ? iMouse.xy / iResolution.xy  : uv.xy;
    
    	 if (e.x < .2500) r = b;
    else if (e.x < .5000) r = d;
    else
    {
   		     if (e.y < .3333) r = co;
   		else if (e.y < .6666) r = cg;
   		else                  r = y;
    }

#ifdef TEXT
    if (Text(uv) > 0.) r = vec3(1,0,0);
#endif // TEXT
