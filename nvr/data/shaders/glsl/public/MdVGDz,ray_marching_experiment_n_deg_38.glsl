// Shader downloaded from https://www.shadertoy.com/view/MdVGDz
// written by shadertoy user aiekick
//
// Name: Ray Marching Experiment n&deg;38
...
vec3 effect(vec2 v) 
{
    v = abs(fract(vec2(1.5,-.5) * (length(v) - iDate.w) 
        + atan(v.x, v.y) * 0.477)-.5);
	return abs(max(v.x*1.2+v.y*6., v.x*1.2) - 1.4) 
        * vec3(.09,.14,.22)/length(v);
}
