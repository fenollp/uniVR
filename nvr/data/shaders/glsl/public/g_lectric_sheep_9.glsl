// Shader downloaded from https://www.shadertoy.com/view/XdtXWl
// written by shadertoy user vox
//
// Name: G-Lectric Sheep 9
// Description: G-Lectric Sheep 9
//-----------------SETTINGS-----------------
...
#define time ((saw(float(__LINE__))+1.0)*(iGlobalTime)/PI/2.0)
...
vec2 mobius(vec2 uv)
{
	vec2 a = vec2(MOUSE_X, MOUSE_Y)*2.0*PI;//sin(seedling+vec2(time, time*GR/E));
...
{
    float turns = 2.0+1.0*sin(time);
    float r = exp(log(length(uv)+1.0));
    float theta = atan(uv.y, uv.x)*turns+r*PI*sin(time)*4.0;//+r*PI*turns;
    return vec2(saw(r*PI+PI/2.0),
                saw(theta));
}

vec2 spiral(vec2 uv)
{
    float turns = 5.0+1.0*sin(time);
    float r = exp(log(length(uv)+1.0));
    float theta = atan(uv.y, uv.x)*turns+r*PI*(saw(time)+1.0)*2.0;//+r*PI*turns;
    return vec2(saw(r*PI+PI/2.0),
                saw(theta));
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    float shift = time;
    //depth /= float(max_i);
    float stripes = depth*2.0*PI*floor(log(max(iResolution.x, iResolution.y))/log(2.0));
