// Shader downloaded from https://www.shadertoy.com/view/4dyXzw
// written by shadertoy user soma_arc
//
// Name: Hyperbolic triangular tiling
// Description: Tiling with Hyperbolic triangle. Their interior angles are (PI/2, PI/4, PI/8). We use the Algorithm which called Iterated Inversion System (IIS) developed with Kazushi Ahara (Meiji University).
/*
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
    ...
    for(float i = 0. ; i < sampleNum ; i++){
        ...
        if(d == 0){
            ...
            if(mod(float(d), 2.) == 0.){
                sum += hsv2rgb(vec3(0.001 * colCount, 1., 1.));
            }else{
                sum += hsv2rgb(vec3(0.7 + 0.001 * colCount, 1., 1.));
            }
        }
    }
