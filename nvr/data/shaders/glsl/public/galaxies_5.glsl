// Shader downloaded from https://www.shadertoy.com/view/ldySzR
// written by shadertoy user vox
//
// Name: Galaxies 5
// Description: Galaxies 5
//-----------------SETTINGS-----------------
#define TIMES_DETAILED (1.125)
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    for(int i = 0; i < max_i; i++)
    {
        ...
        vec3 sample = texture2D(iChannel0, uv).rgb;
    	float sound =(simplex3d(sample*vec3(uv*2.0-1.0+fragCoord.xy/iResolution.xy*2.0-1.0, depth)))*length(sample);
        seedling += sound*(1.0-ifs);
    ...
    float stripes = depth+simplex3d(vec3(fragCoord.xy/iResolution.xy+uv, time))+shift;
    ...
    fragColor = vec4(final, 1.0)*sound*2.0;
