// Shader downloaded from https://www.shadertoy.com/view/MstSD8
// written by shadertoy user reinder
//
// Name: Matrix Lobby Scene (MP)
// Description: This is my Sig15 shader [url=https://www.shadertoy.com/view/MtsXzf][SIG15] Matrix Lobby Scene[/url]. I don't know why, but the original shader keeps crashing webgl on some windows machines. Therefore I made this version which uses multiplepass.
// Original shader. Instead of calculating the first intersection, depth is sampled from buffer A.
...
float intersect( in vec3 ro, in vec3 rd, in vec2 uv ) {
    vec4 dist = texture2D(iChannel3, uv);
