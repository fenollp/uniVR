// Shader downloaded from https://www.shadertoy.com/view/lsy3zw
// written by shadertoy user unconed
//
// Name: Wildstyle SDF
// Description: Mixing folds with the original position to distort a base shape.
vec4 fold1(vec4 z) {
    vec3 p = z.xyz;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec3 n = normal(p);
    if (t < tmax && d < dmin) {
      vec3 albedo = mix(vec3(1., .5, .1), vec3(1.0), surface);
