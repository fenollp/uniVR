// Shader downloaded from https://www.shadertoy.com/view/XdcSDN
...
{
	float sample = texture2DLodEXT(iChannel0, uv, 0.0).g;
	return sample * MAX_HEIGHT;
}
...
vec3 lighting(in vec3 N, in vec3 V, in vec3 reflectance)
{
    ...
    vec3 diffuse = (textureCube(iChannel2, N).rgb * LIGHT_STRENGTH) * NdotV;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    vec3 pos = vec3(cos(iGlobalTime), 1.2 + sin(iGlobalTime) * 0.5, 3.0);
