// Shader downloaded from https://www.shadertoy.com/view/XsyGDc
// written by shadertoy user dr2
//
// Name: Stone Spiral
// Description: Falling stones (mouse enabled); automatically restarts and each run is different.
// "Stone Spiral" by dr2 - 2016
...
const vec3 blkGap = vec3 (1., 0.8, 0.4);
...
vec3 BgCol (vec3 ro, vec3 rd, float sh)
{
  ...
  if (rd.y >= 0.) {
    ...
    w = 0.2 * ro.xz;
    f = Fbm2 (w);
    col = mix (vec3 (0., 0.3, 0.), vec3 (0.2, 0.7, 0.2), f) *
         (1. - 0.1 * Noisefv2 (w));
...
vec3 ShowScene (vec3 ro, vec3 rd)
{
  ...
  if (min (dstBlk, dstFlr) < dstFar) {
    if (dstFlr < dstBlk) {
      ...
      ro += rd * dstBlk;
      vn = vnBlk;
      objCol = vec4 (vec3 (0.6, 0.6, 0.7) * (1. - 0.4 * Fbm2 (20. * qBlk)), 0.5);
    }
