// Shader downloaded from https://www.shadertoy.com/view/ld3SDl
// written by shadertoy user cornusammonis
//
// Name: Fast Thin-Film Interference
// Description: A fast and fairly realistic thin-film interference and chromatic dispersion simulation shader.
/*
...
vec3 sampleCubeMap(vec3 i, vec3 rd) {
	vec3 col = textureCube(iChannel0, rd * vec3(1.0,-1.0,1.0), 0.0).xyz; 
    return vec3(
...
vec3 sampleCubeMap(vec3 i, vec3 rd0, vec3 rd1, vec3 rd2) {
	vec3 col0 = textureCube(iChannel0, rd0 * vec3(1.0,-1.0,1.0), 0.0).xyz;
    vec3 col1 = textureCube(iChannel0, rd1 * vec3(1.0,-1.0,1.0), 0.0).xyz; 
    vec3 col2 = textureCube(iChannel0, rd2 * vec3(1.0,-1.0,1.0), 0.0).xyz; 
    return vec3(
