// Shader downloaded from https://www.shadertoy.com/view/ldVSRR
// written by shadertoy user vox
//
// Name: Dancing Galaxies 3
// Description: Dancing Galaxies 3
//-----------------SETTINGS-----------------
...
float lowAverage()
{
    ...
    {
        next = length(texture2D(iChannel0, vec2(float(i)/float(iters), 0.0)).r);
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    float swirl = clamp(saw(time)*3.0-1.0, 0.0, 1.0);
    float scale = 2.0*PI;
    uv *= pow(scale, swirl);
    uv -= scale/2.0*swirl;
	uv.x *= pow(iResolution.x/iResolution.y, swirl);
    uv += scale*vec2(cos(iGlobalTime*.234), sin(iGlobalTime*.345))*1.0*swirl;
    
    uv = spiral(uv)*swirl+uv*(1.0-swirl);
    ...
    if(pow(ifs, 1.0/float(last_i+1)) > .75-height)
    {
        if( sound < .25)
            sound = 0.0;
        else
