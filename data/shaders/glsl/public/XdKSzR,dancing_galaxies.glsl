// Shader downloaded from https://www.shadertoy.com/view/XdKSzR
// written by shadertoy user vox
//
// Name: Dancing Galaxies
// Description: Dancing Galaxies
//-----------------SETTINGS-----------------
...
vec2 mobius(vec2 uv)
{
	vec2 a = sin(seedling*.10*PI+vec2(time*1.0*PI, time*GR/E*1.0*PI))*.25+.5;
	vec2 b = sin(seedling*.20*PI+vec2(time*1.0*PI, time*GR/E*1.0*PI));
	vec2 c = sin(seedling*.30*PI+vec2(time*1.0*PI, time*GR/E*1.0*PI))*.25+.5;
	vec2 d = sin(seedling*.40*PI+vec2(time*1.0*PI, time*GR/E*1.0*PI));
	return cdiv(cmul(uv, a) + b, cmul(uv, c) + d);
}

vec2 map(vec2 uv)
{
    return saw(mobius(zoom*(uv*2.0-1.0))*PI);
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    if(pow(ifs, 1.0/float(last_i+1)) > .125+.0625*sin(time*3.0*PI))
    {
        if(sound < .125)
            fragColor *= sound;
        else
    ...
    sound = texture2D(iChannel0, uv).r;
    fragColor = vec4(final, 1.0)*clamp(sound*12.0, 0.0, 1.0);
