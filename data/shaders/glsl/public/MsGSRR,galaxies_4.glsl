// Shader downloaded from https://www.shadertoy.com/view/MsGSRR
// written by shadertoy user vox
//
// Name: Galaxies 4
// Description: Galaxies 4
//-----------------SETTINGS-----------------
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    for(int i = 0; i < max_i; i++)
    {
        ...
        float weight = ifs;//smoothstep(0.0, 0.5, ifs);
    ...
    if(pow(ifs, 1.0/float(last_i+1)) > .125*height*sound)
    {
        if( sound < .125)
            sound = 0.0;
        else
    ...
    //depth /= float(max_i);
    float shift = PI*height*8.0;

    float stripes = depth*1.0+sound;
    float black = smoothstep(0.0, .75, saw(stripes))*smoothstep(0.0, .1, sound);;
